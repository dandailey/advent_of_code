...
class Octogrid
  ...
  def octopus_count
    @octopus_count ||= grid.flatten.length
  end
...
class Solver
  def calculate_answer
    ...
    sync_step = (1..100000).each do |step|
      octogrid = timeline.octogrid_at_step(step)
      break step if octogrid.flash_count == octogrid.octopus_count
    end
    sync_step
  end
end
