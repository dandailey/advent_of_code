class Solver
  ...
  def calculate_answer
    ...
    called_numbers.each do |called_number|
      ...
      losers = board_list.each_with_index.map { |board, board_index|
        board_is_winner?(board_index) ? nil : board_index
      }.compact
      log "#{losers.length} losers"

      if losers.length == 1
        last_winning_board_index = losers.first
      end

      if losers.length == 0
        return called_number.to_i * board_list[last_winning_board_index].compact.map(&:to_i).sum
      end
    end
  end
end
