...
HEREDOC
  ...
  def answer
    range = first_contiguous_range_adding_to_first_invalid_entry
    range.min + range.max
  end
  ...
  def first_contiguous_range_adding_to_first_invalid_entry
    @first_contiguous_range_adding_to_first_invalid_entry ||= nil
    if @first_contiguous_range_adding_to_first_invalid_entry.nil?
      looking_for = first_invalid_entry
      (0..(number_list.length - 2)).each do |i|
        (i..(number_list.length - 1)).each do |j|
          sub_list = number_list[i, j - i + 1]
          if sub_list.sum == looking_for
            @first_contiguous_range_adding_to_first_invalid_entry = sub_list
            break
          end
        end
        break if @first_contiguous_range_adding_to_first_invalid_entry
      end
    end
    @first_contiguous_range_adding_to_first_invalid_entry
  end
